...
async def dumtyer(dumtter):
    await dumtter.edit("⊙ g∂яιvε ⊙"
                       ...
                       "\n⊙ .gd : Upload file from local or uri/url/drivelink into google drive. "
                       "for drivelink it's upload only if you want to."
                       "\n⊙ .gdabort : Abort process uploading or downloading."
                       "\n⊙ .gdlist : Get list of folders and files with default size 50."
                       "\n- Use flags `-l range[1-1000]` for limit output."
                       "\n- Use flags `-p parents-folder_id` for lists given folder in gdrive."
                       "\n⊙ .gdf mkdir : Create gdrive folder."
                       "\n⊙ .gdf chck : Check file/folder in gdrive."
