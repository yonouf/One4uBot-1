...
@register(outgoing=True, pattern=r"^\.son (?:(now)|(.*) - (.*))")
async def _(event):
    if event.fwd_from:
        return
...
@register(outgoing=True, pattern=r"^\.xsongl(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return
...
@register(outgoing=True, pattern=r"^\.sof (?:(now)|(.*) - (.*))")
async def _(event):
    if event.fwd_from:
        return
    if event.pattern_match.group(1) == "now":
...
@register(outgoing=True, pattern=r"^\.xvsong(?: |$)(.*)")
async def _(event):
